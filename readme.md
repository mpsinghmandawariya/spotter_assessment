...
| **Email** | [mmahipalsingh717@gmail.com](mailto:mmahipalsingh717@gmail.com) |
| **Mobile** | +91 96864 44115 |
| **LinkedIn** | [linkedin.com/in/mahipal-singh-deora](https://www.linkedin.com/in/mahipal-singh-deora-8703a9313/) |
| **GitHub** | [github.com/mpsinghmandawariya](https://github.com/mpsinghmandawariya) |

## 🔗 Repository

[github.com/mpsinghmandawariya/spotter_assessment](https://github.com/mpsinghmandawariya/spotter_assessment)
...
4. [Original Features](#4-original-features-14-columns)
5. [Exploratory Data Analysis](#5-exploratory-data-analysis)
6. [Feature Engineering](#6-feature-engineering)
7. [Data Quality Handling](#7-data-quality-handling)
8. [Baseline Modeling](#8-baseline-modeling)
9. [Error Analysis](#9-error-analysis)
10. [XGBoost Experiments](#10-xgboost-experiments)
11. [High-Rate Analysis](#11-high-rate-analysis)
12. [Refined Feature Experiment](#12-refined-feature-experiment)
13. [Feature Importance](#13-feature-importance)
14. [Controlled Hyperparameter Tuning](#14-controlled-hyperparameter-tuning)
15. [Rolling Time Validation](#15-rolling-time-validation)
16. [Robustness Experiment](#16-robustness-experiment)
17. [Final Model](#17-final-model)
18. [Final Prediction Results](#18-final-prediction-results)
19. [Final Submission Validation](#19-final-submission-validation)
20. [Custom Prediction](#20-custom-prediction)
21. [Known Limitation](#21-known-limitation)
22. [Project Structure](#22-project-structure)
23. [Script Execution Order](#23-script-execution-order)
24. [How to Run](#24-how-to-run)
25. [Evaluation Metrics](#25-evaluation-metrics)
26. [Why Time-Based Validation Matters](#26-why-time-based-validation-matters)
27. [Final Model Selection Logic](#27-final-model-selection-logic)
28. [Final Results Summary](#28-final-results-summary)
29. [Summary](#29-summary)
30. [Submission Checklist](#30-submission-checklist)
31. [Final Files](#31-final-files)
...
flowchart TD
    ...
    D --> E[Baseline Models]
    E --> F[Error Analysis]
    F --> G[XGBoost Experiments]
    G --> H[Refined Features]
    H --> I[High-Rate Analysis]
    I --> J[Feature Importance]
    J --> K[Hyperparameter Tuning]
    K --> L[Rolling Time Validation]
    L --> M[Robustness Experiments]
    M --> N[Final Model Training]
    N --> O[12,000 Predictions]
    O --> P[Submission Validation]
    P --> Q[Custom Prediction]
...
![Distance vs Posted Rate](outputs/eda/04_distance_vs_rate.png)
*Distance is clearly the dominant driver — the tight lower band is normal-rate loads; the scattered upper cloud is the rare high-rate loads discussed in [Section 11](#11-high-rate-analysis).*

![Average Posted Rate by Equipment](outputs/eda/08_equipment_vs_rate.png)

![Monthly Average Posted Rate](outputs/eda/11_monthly_rate_trend.png)
...
```mermaid
xychart-beta
    title "MAE by Distance Band ($)"
    x-axis ["0-100", "100-250", "250-500", "500-750", "750-1000", "1000-1500", "1500-2500", "2500+"]
    y-axis "MAE ($)" 0 --> 300
    bar [23.79, 37.77, 71.67, 92.28, 111.15, 204.29, 264.55, 269.33]
```

Error grows substantially with distance — long-haul shipments are harder to estimate.

![Absolute Prediction Error vs Distance](outputs/error_analysis/03_error_vs_distance.png)
*The handful of large outliers are the same rare high-rate loads flagged in the EDA above.*
...
```mermaid
pie showData title Aggregated Feature Importance
    "Distance" : 74.9
    "Route" : 14.6
    "Delivery" : 3.7
    "Pickup" : 3.7
...
**Example input:** Phoenix → Los Angeles, Dry Van, 370 mi, 30,000 lbs, 2025-10-15, market index 100, quote signal 0.5
**Example output:** Predicted Freight Rate: **$873.28**
...
```mermaid
flowchart LR
    S1[01_data_audit] --> S2[02_eda] --> S3[03_features] --> S4[04_baseline_models]
    S4 --> S5[05_error_analysis] --> S6[06_boosting_models] --> S7[07_high_rate_analysis]
    S7 --> S8[08_refined_features] --> S9[09_test_refined_xgboost] --> S10[10_two_stage_model]
    S10 --> S11[11_tune_high_rate_strategy] --> S12[12_feature_importance] --> S13[13_xgboost_tuning]
    S13 --> S14[14_rolling_time_validation] --> S15[15_residual_analysis] --> S16[17_robust_xgboost]
    S16 --> S17[18_final_train] --> S18[19_validate_submission] --> S19[20_final_model_summary]
    S19 --> S20[21_custom_prediction]

    style S17 fill:#dcfce7,stroke:#15803d
    style S18 fill:#dcfce7,stroke:#15803d
```
...
```mermaid
flowchart TD
    A[Baseline Models] --> B[Identify Nonlinear Behavior]
    B --> C[Test XGBoost]
    C --> D[Compare Raw vs. Log Target]
    D --> E[Analyze Errors]
    E --> F[Investigate Extreme Rates]
    F --> G[Test Refined Features]
    G --> H[Measure Feature Importance]
    H --> I[Tune XGBoost]
    I --> J[Rolling Validation]
    J --> K[Robustness Testing]
    K --> L{{Select Champion}}
...
```mermaid
xychart-beta
    title "Time-Split MAE by Model ($, lower is better)"
    x-axis ["Ridge", "RF", "XGB-Raw", "XGB-Log", "Refined XGB", "Tuned XGB (Champion)"]
    y-axis "MAE ($)" 0 --> 350
    bar [322.62, 157.98, 141.96, 135.95, 138.60, 127.14]
```
...
- [x] `outputs/final_model/final_predictions.csv` exists (12,000 rows)
- [x] `load_id` / `predicted_rate` columns present in the required format
- [x] `outputs/final_model/submission_audit.csv` confirms no duplicate, missing, infinite, or negative predictions
- [x] Final model saved (`outputs/final_model/final_xgboost_model.pkl`)
- [x] README included
- [x] Experiment history preserved (all 20 numbered scripts + `outputs/`)
- [x] Custom prediction script included (`21_custom_prediction.py`)
...
**Mahipal Singh Deora**
📧 [mmahipalsingh717@gmail.com](mailto:mmahipalsingh717@gmail.com) · 📱 +91 96864 44115
🔗 [LinkedIn](https://www.linkedin.com/in/mahipal-singh-deora-8703a9313/) · 💻 [GitHub](https://github.com/mpsinghmandawariya)
